  ...
  return (
    <tr key={item.id} className={cx({ [classes.rowSelected]: selected })}>
      ...
      <td onClick={() => toggleRow(item.id)}>
        <Checkbox readOnly checked={selected} transitionDuration={0} />
      </td>
      {/* Edit Action */}
      <td>
        <ActionIcon
      ...
      <td>
        <Avatar size={26} radius={26} color="cyan">
          B
        </Avatar>
      </td>
      <td>
      ...
      <td>
        <Group spacing="sm">
          ...
          <Tooltip label={'Visit'}>
            <ActionIcon color="blue" variant="light" size={'sm'} component="a" href={item.url} target="_blank">
      ...
      {/* Redirect Links */}
      <td>
        <IconCheck checked={item.app_url} />
      </td>
      <td>
        <IconCheck checked={item.android_url} />
      </td>
      <td>
        <IconCheck checked={item.ios_url} />
      </td>
      <td>
        <IconCheck checked={item.fallback_url} />
      </td>
      <td>{item.visits}</td>
    </tr>
  );
